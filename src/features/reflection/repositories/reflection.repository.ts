...
export class ReflectionRepository {
  ...
  async getHistoryArchive(): Promise<DailyHistoryRecord[]> {
    ...
    logRows.forEach((r: { date_key: string; count: number }) => logsMap.set(r.date_key, r.count));
    ...
    logRows.forEach((r: { date_key: string }) => allDates.add(r.date_key));
    reflectionRows.forEach((r: { date_key: string }) => allDates.add(r.date_key));

    const reflectionsMap = new Map<string, { content: string; heart_state: string }>();
    reflectionRows.forEach((r: { date_key: string; content: string; heart_state?: string }) => reflectionsMap.set(r.date_key, { content: r.content, heart_state: r.heart_state || '' }));
